...
class MinerManagerCLI:
    ...
    async def evaluate_local(self) -> None:
        ...
        dataset_manager = LocalDatasetManager(
            self.config, str(self.config.competition_id)
        )
        ...
        competition_handler = COMPETITION_HANDLER_MAPPING[
            str(self.config.competition_id)
        ...
        self.loop_metrics.append(model_result.score)
        print(f"Current loop metrics: {self.loop_metrics}")
